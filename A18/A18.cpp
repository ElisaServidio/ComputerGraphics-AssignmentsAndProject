...
std::vector<float> M1_InternalVertices;
std::vector<uint32_t> M1_indices;	
std::vector<float> M2_InternalVertices;
std::vector<uint32_t> M2_indices;	
std::vector<float> M3_InternalVertices;
std::vector<uint32_t> M3_indices;	
std::vector<float> M4_InternalVertices;
std::vector<uint32_t> M4_indices;	
int Mstart[4], Mlen[4], MlenNorm[4];
...
private:
	...
	VertexDescriptor 	geometryVertices = VertexDescriptor(true, true, false, false, false);
	...
	glm::vec3 CamPos = glm::vec3(4.0f, 1.5f, 4.5f);
	glm::vec3 CamAng = glm::vec3(-0.12f, 0.66f, 0.0f);
	...
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<Assignment18*>
...
//std::cout << k << " " << j << " " << i << " " << vb << " " << ib << " " << c << "\n";
	...
	void createModelMesh(ModelData& MD, VertexDescriptor &VD) {
		...
		const float deltaScale = 0.2;
		M1_InternalVertices.resize(2 * 6 * M1_vertices.size());
		for(int i = 0; i <  M1_vertices.size(); i++) {
			int secondStart = 6 * M1_vertices.size();
			M1_InternalVertices[i*6+0] = M1_vertices[i].pos.x;
			M1_InternalVertices[i*6+1] = M1_vertices[i].pos.y;
			M1_InternalVertices[i*6+2] = M1_vertices[i].pos.z;
			M1_InternalVertices[i*6+3] = M1_vertices[i].norm.x;
			M1_InternalVertices[i*6+4] = M1_vertices[i].norm.y;
			M1_InternalVertices[i*6+5] = M1_vertices[i].norm.z;
			M1_InternalVertices[secondStart+i*6+0] = M1_vertices[i].pos.x + deltaScale * M1_vertices[i].norm.x;
			M1_InternalVertices[secondStart+i*6+1] = M1_vertices[i].pos.y + deltaScale * M1_vertices[i].norm.y;
			M1_InternalVertices[secondStart+i*6+2] = M1_vertices[i].pos.z + deltaScale * M1_vertices[i].norm.z;
			M1_InternalVertices[secondStart+i*6+3] = M1_vertices[i].norm.x;
			M1_InternalVertices[secondStart+i*6+4] = M1_vertices[i].norm.y;
			M1_InternalVertices[secondStart+i*6+5] = M1_vertices[i].norm.z;
		}
		M2_InternalVertices.resize(2 * 6 * M2_vertices.size());
		for(int i = 0; i <  M2_vertices.size(); i++) {
			int secondStart = 6 * M2_vertices.size();
			M2_InternalVertices[i*6+0] = M2_vertices[i].pos.x;
			M2_InternalVertices[i*6+1] = M2_vertices[i].pos.y;
			M2_InternalVertices[i*6+2] = M2_vertices[i].pos.z;
			M2_InternalVertices[i*6+3] = M2_vertices[i].norm.x;
			M2_InternalVertices[i*6+4] = M2_vertices[i].norm.y;
			M2_InternalVertices[i*6+5] = M2_vertices[i].norm.z;
			M2_InternalVertices[secondStart+i*6+0] = M2_vertices[i].pos.x + deltaScale * M2_vertices[i].norm.x;
			M2_InternalVertices[secondStart+i*6+1] = M2_vertices[i].pos.y + deltaScale * M2_vertices[i].norm.y;
			M2_InternalVertices[secondStart+i*6+2] = M2_vertices[i].pos.z + deltaScale * M2_vertices[i].norm.z;
			M2_InternalVertices[secondStart+i*6+3] = M2_vertices[i].norm.x;
			M2_InternalVertices[secondStart+i*6+4] = M2_vertices[i].norm.y;
			M2_InternalVertices[secondStart+i*6+5] = M2_vertices[i].norm.z;
		}
		M3_InternalVertices.resize(2 * 6 * M3_vertices.size());
		for(int i = 0; i <  M3_vertices.size(); i++) {
			int secondStart = 6 * M3_vertices.size();
			M3_InternalVertices[i*6+0] = M3_vertices[i].pos.x;
			M3_InternalVertices[i*6+1] = M3_vertices[i].pos.y;
			M3_InternalVertices[i*6+2] = M3_vertices[i].pos.z;
			M3_InternalVertices[i*6+3] = M3_vertices[i].norm.x;
			M3_InternalVertices[i*6+4] = M3_vertices[i].norm.y;
			M3_InternalVertices[i*6+5] = M3_vertices[i].norm.z;
			M3_InternalVertices[secondStart+i*6+0] = M3_vertices[i].pos.x + deltaScale * M3_vertices[i].norm.x;
			M3_InternalVertices[secondStart+i*6+1] = M3_vertices[i].pos.y + deltaScale * M3_vertices[i].norm.y;
			M3_InternalVertices[secondStart+i*6+2] = M3_vertices[i].pos.z + deltaScale * M3_vertices[i].norm.z;
			M3_InternalVertices[secondStart+i*6+3] = M3_vertices[i].norm.x;
			M3_InternalVertices[secondStart+i*6+4] = M3_vertices[i].norm.y;
			M3_InternalVertices[secondStart+i*6+5] = M3_vertices[i].norm.z;
		}
		M4_InternalVertices.resize(2 * 6 * M4_vertices.size());
		for(int i = 0; i <  M4_vertices.size(); i++) {
			int secondStart = 6 * M4_vertices.size();
			M4_InternalVertices[i*6+0] = M4_vertices[i].pos.x;
			M4_InternalVertices[i*6+1] = M4_vertices[i].pos.y;
			M4_InternalVertices[i*6+2] = M4_vertices[i].pos.z;
			M4_InternalVertices[i*6+3] = M4_vertices[i].norm.x;
			M4_InternalVertices[i*6+4] = M4_vertices[i].norm.y;
			M4_InternalVertices[i*6+5] = M4_vertices[i].norm.z;
			M4_InternalVertices[secondStart+i*6+0] = M4_vertices[i].pos.x + deltaScale * M4_vertices[i].norm.x;
			M4_InternalVertices[secondStart+i*6+1] = M4_vertices[i].pos.y + deltaScale * M4_vertices[i].norm.y;
			M4_InternalVertices[secondStart+i*6+2] = M4_vertices[i].pos.z + deltaScale * M4_vertices[i].norm.z;
			M4_InternalVertices[secondStart+i*6+3] = M4_vertices[i].norm.x;
			M4_InternalVertices[secondStart+i*6+4] = M4_vertices[i].norm.y;
			M4_InternalVertices[secondStart+i*6+5] = M4_vertices[i].norm.z;
		}
		
		MD.vertices.resize(M1_InternalVertices.size() + M2_InternalVertices.size() + M3_InternalVertices.size() + M4_InternalVertices.size());
		MD.indices.resize(M1_indices.size() + M2_indices.size() + M3_indices.size() + M4_indices.size()
		 + 3 * (M1_InternalVertices.size() + M2_InternalVertices.size() + M3_InternalVertices.size() + M4_InternalVertices.size()));
		
		int stV = 0, stI = 0;
		for(int i = 0; i < M1_InternalVertices.size(); i++) {
			MD.vertices[stV+i] = M1_InternalVertices[i];
		}
		for(int i = 0; i < M1_indices.size(); i++) {
			MD.indices[stI+i] = M1_indices[i] + stV/6;
		}
		Mstart[0] = stI;
		Mlen[0] = M1_indices.size();
		stI += M1_indices.size();
		for(int i = 0; i < M1_InternalVertices.size(); i++) {
			int secondStart = M1_vertices.size();
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i+secondStart + stV/6;
		}
		MlenNorm[0] = M1_indices.size()+M1_InternalVertices.size()/4;
		stV += M1_InternalVertices.size();		

		for(int i = 0; i < M2_InternalVertices.size(); i++) {
			MD.vertices[stV+i] = M2_InternalVertices[i];
		}
		for(int i = 0; i < M2_indices.size(); i++) {
			MD.indices[stI+i] = M2_indices[i] + stV/6;
		}
		Mstart[1] = stI;
		Mlen[1] = M2_indices.size();
		stI += M2_indices.size();
		for(int i = 0; i < M2_InternalVertices.size(); i++) {
			int secondStart = M2_vertices.size();
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i+secondStart + stV/6;
		}
		MlenNorm[1] = M2_indices.size()+M2_InternalVertices.size()/4;
		stV += M2_InternalVertices.size();		

		for(int i = 0; i < M3_InternalVertices.size(); i++) {
			MD.vertices[stV+i] = M3_InternalVertices[i];
		}
		for(int i = 0; i < M3_indices.size(); i++) {
			MD.indices[stI+i] = M3_indices[i] + stV/6;
		}
		Mstart[2] = stI;
		Mlen[2] = M3_indices.size();
		stI += M3_indices.size();
		for(int i = 0; i < M3_InternalVertices.size(); i++) {
			int secondStart = M3_vertices.size();
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i+secondStart + stV/6;
		}
		MlenNorm[2] = M3_indices.size()+M3_InternalVertices.size()/4;
		stV += M3_InternalVertices.size();		

		for(int i = 0; i < M4_InternalVertices.size(); i++) {
			MD.vertices[stV+i] = M4_InternalVertices[i];
		}
		for(int i = 0; i < M4_indices.size(); i++) {
			MD.indices[stI+i] = M4_indices[i] + stV/6;
		}
		Mstart[3] = stI;
		Mlen[3] = M4_indices.size();
		stI += M4_indices.size();
		for(int i = 0; i < M4_InternalVertices.size(); i++) {
			int secondStart = M4_vertices.size();
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i + stV/6;
			MD.indices[stI++] = i+secondStart + stV/6;
		}
		MlenNorm[3] = M4_indices.size()+M4_InternalVertices.size()/4;
		stV += M4_InternalVertices.size();		
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			}
			{
				...
				vkCmdDrawIndexed(commandBuffers[i],
							static_cast<uint32_t>(xray ? MlenNorm[curText] : Mlen[curText]), 1, static_cast<uint32_t>(Mstart[curText]), 0, 0);
			}
...
// std::cout << "Cam Pos: " << CamPos[0] << " " << CamPos[1] << " " << CamPos[2] << "\n";
		...
		gubo.lightDir = glm::vec3(cos(glm::radians(150.0f)) * cos(glm::radians(30.0f)) , sin(glm::radians(150.0f)) * cos(glm::radians(30.0f)), sin(glm::radians(30.0f)));
		gubo.lightColor = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
		gubo.eyePos = CamPos;
		if(mode == 2) {
			gubo.extras = glm::vec4(0.0f, 1.0f, 0.0f, 0.0f);
		} else {
			gubo.extras = glm::vec4(1.0f, 0.0f, 0.0f, 0.0f);
		}
