...
int Mstart[2], Mlen[2], MlenNorm[2];
...
private:
	...
	VertexDescriptor 	geometryVertices = VertexDescriptor(true, true, true, false, false);
	...
	static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
		auto app = reinterpret_cast<Assignment19*>
...
//throw std::runtime_error("Now We Stop Here!");			
	...
	void createModels() {
		...
		createTextureImage("TextureCG2022.png", Scene[0].TD);
		createTextureImageView(Scene[0].TD);
		createTextureSampler(Scene[0].TD);
	}
...
//std::cout << k << " " << j << " " << i << " " << vb << " " << ib << " " << c << "\n";
	...
	void createModelMesh(ModelData& MD, VertexDescriptor &VD) {
		...
		M1_InternalVertices.resize(2 * 8 * M1_vertices.size());
		for(int i = 0; i <  M1_vertices.size(); i++) {
			int secondStart = 8 * M1_vertices.size();
			M1_InternalVertices[i*8+0] = M1_vertices[i].pos.x;
			M1_InternalVertices[i*8+1] = M1_vertices[i].pos.y;
			M1_InternalVertices[i*8+2] = M1_vertices[i].pos.z;
			M1_InternalVertices[i*8+3] = M1_vertices[i].norm.x;
			M1_InternalVertices[i*8+4] = M1_vertices[i].norm.y;
			M1_InternalVertices[i*8+5] = M1_vertices[i].norm.z;
			M1_InternalVertices[i*8+6] = M1_vertices[i].UV.x;
			M1_InternalVertices[i*8+7] = M1_vertices[i].UV.y;
			M1_InternalVertices[secondStart+i*8+0] = M1_vertices[i].pos.x + deltaScale * M1_vertices[i].norm.x;
			M1_InternalVertices[secondStart+i*8+1] = M1_vertices[i].pos.y + deltaScale * M1_vertices[i].norm.y;
			M1_InternalVertices[secondStart+i*8+2] = M1_vertices[i].pos.z + deltaScale * M1_vertices[i].norm.z;
			M1_InternalVertices[secondStart+i*8+3] = M1_vertices[i].norm.x;
			M1_InternalVertices[secondStart+i*8+4] = M1_vertices[i].norm.y;
			M1_InternalVertices[secondStart+i*8+5] = M1_vertices[i].norm.z;
			M1_InternalVertices[secondStart+i*8+6] = M1_vertices[i].UV.x;
			M1_InternalVertices[secondStart+i*8+7] = M1_vertices[i].UV.y;
		}
		M2_InternalVertices.resize(2 * 8 * M2_vertices.size());
		for(int i = 0; i <  M2_vertices.size(); i++) {
			int secondStart = 8 * M2_vertices.size();
			M2_InternalVertices[i*8+0] = M2_vertices[i].pos.x;
			M2_InternalVertices[i*8+1] = M2_vertices[i].pos.y;
			M2_InternalVertices[i*8+2] = M2_vertices[i].pos.z;
			M2_InternalVertices[i*8+3] = M2_vertices[i].norm.x;
			M2_InternalVertices[i*8+4] = M2_vertices[i].norm.y;
			M2_InternalVertices[i*8+5] = M2_vertices[i].norm.z;
			M2_InternalVertices[i*8+6] = M2_vertices[i].UV.x;
			M2_InternalVertices[i*8+7] = M2_vertices[i].UV.y;
			M2_InternalVertices[secondStart+i*8+0] = M2_vertices[i].pos.x + deltaScale * M2_vertices[i].norm.x;
			M2_InternalVertices[secondStart+i*8+1] = M2_vertices[i].pos.y + deltaScale * M2_vertices[i].norm.y;
			M2_InternalVertices[secondStart+i*8+2] = M2_vertices[i].pos.z + deltaScale * M2_vertices[i].norm.z;
			M2_InternalVertices[secondStart+i*8+3] = M2_vertices[i].norm.x;
			M2_InternalVertices[secondStart+i*8+4] = M2_vertices[i].norm.y;
			M2_InternalVertices[secondStart+i*8+5] = M2_vertices[i].norm.z;
			M2_InternalVertices[secondStart+i*8+6] = M2_vertices[i].UV.x;
			M2_InternalVertices[secondStart+i*8+7] = M2_vertices[i].UV.y;
		}
		
		MD.vertices.resize(M1_InternalVertices.size() + M2_InternalVertices.size() );
		MD.indices.resize(M1_indices.size() + M2_indices.size()
		 + 3 * (M1_InternalVertices.size() + M2_InternalVertices.size()));
		...
		for(int i = 0; i < M1_indices.size(); i++) {
			MD.indices[stI+i] = M1_indices[i] + stV/8;
		}
		Mstart[0] = stI;
		...
		for(int i = 0; i < M1_InternalVertices.size(); i++) {
			...
			MD.indices[stI++] = i + stV/8;
			MD.indices[stI++] = i + stV/8;
			MD.indices[stI++] = i+secondStart + stV/8;
		}
		MlenNorm[0] = M1_indices.size()+M1_InternalVertices.size()/16*3;
		...
		for(int i = 0; i < M2_indices.size(); i++) {
			MD.indices[stI+i] = M2_indices[i] + stV/8;
		}
		Mstart[1] = stI;
		...
		MlenNorm[1] = M2_indices.size()+M2_InternalVertices.size()/16*3;
